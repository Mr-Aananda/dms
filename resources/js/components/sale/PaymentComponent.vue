    ...
    <div class="col-12">
            ...
            <div class="col-sm-10">
                <div class="input-group">
                    <input
                        ...
                        step="any"
                        type="number"
                        :value="Number.parseFloat(Math.abs(grandTotal)).toFixed(2)"
                        class="form-control"
                        id="grand_total">
                    <div class="input-group-prepend">
                        <div class="input-group-text">৳</div>
        ...
        <div class="mt-3 row">
            ...
            <label for="due" class="col-sm-2 col-form-label">
                {{ partyRemainingBalance >= 0 ? 'Due' : 'Change' }}
            </label>
            <div class="col-sm-10">
                <div class="input-group">
                    <input
...
let labourCost = ref(0)
let transportCost = ref(0)
let paid = ref(0)

let grandTotal = computed(() => {
    return (
      subtotal.value +
      parseFloat(store.customerBalance) -
      discount.value +
...
const initialValues = (oldSale) => {
    if (oldSale) {
        payment_type.value = oldSale?.payments[0]?.cash_id ? 'cash' : 'bank'
        discount.value = oldSale?.discount
        cashId.value = oldSale?.payments[0]?.cash_id
        bankAccountId.value = oldSale?.payments[0]?.bank_account_id
        labourCost.value = oldSale?.labour_cost;
        transportCost.value = oldSale?.transport_cost;
        paid.value = oldSale?.payments[0]?.amount
  }
}
