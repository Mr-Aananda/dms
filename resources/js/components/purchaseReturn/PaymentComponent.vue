            ...
            <div class="col-sm-10">
                <div class="input-group">
                    ...
                    Number.parseFloat(
                        ...
                        + parseFloat(store.supplierBalance || 0)
...
const initialValues = () => {
    const oldPurchaseReturn = props.oldPurchaseReturn
    if (oldPurchaseReturn) {
        payment_type.value = oldPurchaseReturn?.payments[0]?.cash_id ? 'cash' : 'bank'
        adjustment.value = oldPurchaseReturn?.discount
        cashId.value = oldPurchaseReturn?.payments[0]?.cash_id
        bankAccountId.value = oldPurchaseReturn?.payments[0]?.bank_account_id
        paid.value = oldPurchaseReturn?.payments[0]?.amount
