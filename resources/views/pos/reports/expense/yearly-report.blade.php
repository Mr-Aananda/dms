...
<x-app-layout>
    ...
    <div class="widget mb-3">
        ...
        <div class="widget-body collapse {{ request()->search == '1' ? 'show' : '' }}" id="tableSearch">
            <form action="{{ route('report.expense-yearly-report') }}" method="get">
    ...
    <div id="print-widget">
        ...
        <div class="widget">
            <div class="widget-head mb-2">
                <h5>Report: Yearly Expense Report</h5>
                <p><small>{{ count($expenseDetails) }} results found </small></p>
            </div>
            <div class="widget-body">
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        @forelse($expenseDetails as $index => $expense)
                            <tr>
                                <th>{{  $index + 1}}</th>
                                <td>{{ $expense->name }}</td>
                                @foreach ($months as $month)
                                    <td class="text-end">
                                        {{ isset($expense->sum_of_each_month[$month]) ? number_format($expense->sum_of_each_month[$month], 2) : number_format(0, 2) }}
                                    </td>
                                @endforeach
                                <td class="text-end">
                                    {{ number_format($expense->sum_of_each_month->sum(), 2) }}
                                </td>
                            </tr>
                        @empty
                            <tr>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            @foreach ($months as $month)
                                <th class="text-end">
                                    {{ number_format(collect($expenseDetails)->whereNotNull('sum_of_each_month.'.$month)->sum('sum_of_each_month.'.$month), 2) }}
                                </th>
                            @endforeach
                            <th class="text-end">
                                {{ number_format(collect($expenseDetails)->pluck('sum_of_each_month')->flatten()->sum(), 2) }}
                            </th>
                        </tr>
                    </tfoot>
                </table>
